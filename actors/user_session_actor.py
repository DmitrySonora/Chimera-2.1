...
class UserSessionActor(BaseActor):
    ...
    def _determine_generation_mode(
        ...
        from config.settings import MODE_CONFIDENCE_THRESHOLD, MODE_SCORE_NORMALIZATION_FACTOR
        ...
        if max_score == 0:
            return 'talk', MODE_CONFIDENCE_THRESHOLD  # По умолчанию
        
        detected_mode = max(scores, key=scores.get)
        confidence = min(max_score / MODE_SCORE_NORMALIZATION_FACTOR, 1.0)  # Нормализация уверенности
